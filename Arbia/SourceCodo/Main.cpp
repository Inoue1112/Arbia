...
//		m_pPlayer->SetAnimSpeed( 0.01 );
	...
	//m_pExplosion->Init( m_pDevice, m_pDeviceContext );
					...
					if( !m_pPlayer->GetDead() ){
						...
						//自機が攻撃中以外.
								...
								m_vsmpEnemyMgr[i]->GetPointerEnemyCol( j ) ) )
							{
								...
								PlayEffKick( m_pPlayer->GetPosition() );
...
{
	for( char i=0; i<cEFF_KICK_HIT_MAX; i++ ){
		if( !m_pEffect->PlayCheck( m_ehKickHit[i] ) ){

			D3DXVECTOR3 vEffPos = vPos;
			vEffPos.y += fEFF_UP_OFFSET;

			m_ehKickHit[i] = m_pEffect->Play( clsEffects::enEfcType_KICK_HIT, vEffPos );
			m_pEffect->SetScale( m_ehKickHit[i], vEFF_KICK_HIT_SCALE );
			break;
		}
	}
}
void clsMain::PlayEffSlash( D3DXVECTOR3 vPos )
{
	if( !m_pEffect->PlayCheck( m_ehSlashHit ) ){
		...
		m_ehSlashHit = m_pEffect->Play( clsEffects::enEfcType_KICK_HIT, vEffPos );
	}
